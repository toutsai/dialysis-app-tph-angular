...
function queryInjectionOrdersForMonth(db, patientIds, uploadMonth) {
  ...
  return db
    .prepare(
      ...
      WHERE patient_id IN (${placeholders})
        ...
        AND upload_month = ?
      ORDER BY patient_id, order_code, change_date DESC, created_at DESC
    `,
    )
    .all(...patientIds, uploadMonth)
}
...
export function getDailyInjections(db, targetDate, patientIds) {
  ...
  let orders = queryInjectionOrdersForMonth(db, uniquePatientIds, getUploadMonth(targetDate))
  if (orders.length === 0) {
    orders = queryInjectionOrdersForMonth(db, uniquePatientIds, getPreviousMonth(targetDate))
  }
