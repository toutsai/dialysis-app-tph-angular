...
router.post('/exceptions', ...isEditor, async (req, res) => {
  try {
    ...
    // ── 隔日換床（單日 MOVE）走整併路徑 ──────────────────────────────
    // 同病人同日只保留一筆（from 錨定常規原位、to 換最新；拖回原位則取消）、
    ...
    if (isSingleDayMove(data)) {
      ...
      const result = reconcileSingleDayMove(db, data, masterRules, patientsMap, modifiedBy)

      await logAudit('EXCEPTION_RECONCILE', req.user.id, req.user.name, 'schedule_exceptions',
        result.ids[0] || dateStr, { action: result.action, patientName: data.patientName, date: dateStr })
...
/**
 * GET /api/schedules/nurse-assignments/:date/revisions
 * 列出某日的護理分組歷史快照（供異常復原）。
 */
router.get('/nurse-assignments/:date/revisions', ...isEditor, (req, res) => {
  try {
    const { date } = req.params
    const db = getDatabase()
    const rows = db.prepare(`
      SELECT id, date, teams, snapshot_type, created_by, created_at
      FROM nurse_assignment_revisions
      WHERE date = ?
    ...
    const list = rows.map((r) => {
      let assignedCount = 0
      try {
        assignedCount = Object.keys(JSON.parse(r.teams || '{}').teams || {}).length
      } catch {}
      let createdBy = {}
      try {
    ...
    console.error('取得護理分組快照列表錯誤:', error)
    res.status(500).json({ error: true, message: '取得護理分組快照失敗' })
  }
})

/**
 * POST /api/schedules/nurse-assignments/:date/revisions/:revisionId/restore
 * 還原指定快照（還原前先快照現況，使還原本身可再還原）。
 */
router.post('/nurse-assignments/:date/revisions/:revisionId/restore', ...isEditor, async (req, res) => {
  try {
    const { date, revisionId } = req.params
    const db = getDatabase()

    const rev = db
      .prepare(`SELECT * FROM nurse_assignment_revisions WHERE id = ? AND date = ?`)
      .get(revisionId, date)
    if (!rev) {
      return res.status(404).json({ error: true, message: '找不到指定的快照' })
    }

    const runRestore = db.transaction(() => {
      // 還原前先把現況存一份（pre_restore），讓還原可再還原
      snapshotNurseAssignmentForDate(db, date, {
        type: 'pre_restore',
        createdBy: { uid: req.user.id, name: req.user.name },
      })
      db.prepare(`
        ...
        ON CONFLICT(date) DO UPDATE SET
          teams = excluded.teams,
          updated_at = datetime('now', 'localtime')
      `).run(date, date, rev.teams)
    })
    runRestore()

    await logAudit('NURSE_ASSIGNMENT_RESTORE', req.user.id, req.user.name, 'nurse_assignments', date, {
      revisionId,
      snapshotType: rev.snapshot_type,
    })

    const data = (() => {
      try {
        return JSON.parse(rev.teams || '{}')
      } catch {
        return {}
      }
    })()
    res.json({
      ...
      teams: data.teams || {},
      names: data.names || {},
      takeoffEnabled: !!data.takeoffEnabled,
    })
  } catch (error) {
    console.error('還原護理分組錯誤:', error)
    res.status(500).json({ error: true, message: '還原護理分組失敗' })
  }
})
