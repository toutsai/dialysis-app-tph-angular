...
function toDbFormat(data, existingPatient = null) {
  ...
  if (data.hepatitisStatus !== undefined && data.hepatitisStatus !== null) {
    ...
    let baseTags = data.diseases
    if (baseTags === undefined) {
      try { baseTags = JSON.parse(existingPatient?.diseases || '[]') } catch { baseTags = [] }
    }
    result.hepatitis_status = JSON.stringify(status)
    ...
    result.hepatitis_status = JSON.stringify(sameTags ? existingStatus : derived)
  }
...
async function updatePatientHandler(req, res) {
  try {
    ...
    // 完整編輯表單會回傳目前的 isDeleted；先驗證型別，再檢查是否真的改變狀態。
    if (Object.hasOwn(data, 'isDeleted')) {
      if (typeof data.isDeleted !== 'boolean') {
        return res.status(400).json({ error: true, message: 'isDeleted 必須是布林值' })
      }
    }
    ...
    // 刪除／復原的狀態轉換與專用端點採相同角色限制；PATCH 亦會轉到此入口。
    if (Object.hasOwn(data, 'isDeleted') && data.isDeleted !== (existing.is_deleted === 1) && !hasPermission(req.user.role, 'editor')) {
      return res.status(403).json({ error: true, message: '刪除或復原病人需要編輯者權限' })
    }
