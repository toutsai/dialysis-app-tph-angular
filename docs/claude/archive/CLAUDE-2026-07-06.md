...
src/
  ...
  routes/
    ...
    memos.js                   # 備忘錄 CRUD
  services/
    scheduler.js               # node-cron 定時任務 (歸檔/備份/清理/排程初始化/預約變更套用)
    scheduleSync.js            # 總表 -> 未來 60 天排程同步引擎
    exceptionHandler.js        # 調班處理器 (MOVE/SUSPEND/ADD_SESSION/SWAP)
    kiditSync.js               # 工作日誌 -> KiDit 日誌本同步 (KIDIT_EXCLUDED_MOVEMENT_TYPES 過濾)
    dailyLogMovementSync.js    # 病人動態寫入工作日誌 (addAutoMovementToDailyLog) + 連動 KiDit
    patientHistory.js          # 病人歷史/快照共用 (recordPatientHistory/createPatientSnapshot)
  utils/
    dateUtils.js               # 台北時區日期工具 (getTaipeiTodayString 等)
    scheduleUtils.js           # 排程常數與工具 (FREQ_MAP, SHIFTS, getScheduleKey)
    dialysisMode.js            # 透析模式正規化 (normalizeDialysisMode/normalizeDialysisOrdersMode)
    backup.js                  # 資料庫備份
...
- **同步鏈路**: 病人操作 → `addAutoMovementToDailyLog`/`addMovementToDailyLog`（寫工作日誌動態）→ `syncEventsToKiditLogbook`（連動 KiDit）。歷史另由 `recordPatientHistory` 寫入。
- **即時操作** (`routes/patients.js`): 新增/刪除/復原/狀態轉移/更改模式 會即時產生動態 + 歷史。純改頻率/床位**不寫**工作日誌（屬排程規則）。
- **預約變更生效** (`services/scheduler.js applyScheduledPatientUpdates`, 每日 01:00): `UPDATE_STATUS`/`UPDATE_MODE`/`DELETE_PATIENT`/`RESTORE_PATIENT` 套用時，比照即時操作同步工作日誌 + KiDit + 歷史（同步包在 try/catch，失敗不影響任務）。`UPDATE_FREQ`/`UPDATE_BASE_SCHEDULE_RULE`（頻率/床位）**不寫**工作日誌。**歷史只在生效日套用當下寫，預約建立時不寫。**
- **調班** (`services/exceptionHandler.js`): `ADD_SESSION`（臨時加洗）會經 `addAutoMovementToDailyLog` 寫工作日誌動態（type `臨時加洗`），取消時由 `removeAutoMovementFromDailyLog` 移除；其餘調班類型不寫病人動態。
- **⚠️ 刻意決策 — 「更改模式」不進 KiDit**: 由 `kiditSync.js` 的 `KIDIT_EXCLUDED_MOVEMENT_TYPES = Set(['更改模式'])` 在 `syncEventsToKiditLogbook` 過濾。**同時影響即時與預約兩條路徑**（KiDit 是入院/出院/轉床異動申報，模式變更非申報項目；動態仍保留在工作日誌）。勿誤改回。

### 透析模式正規化

`dialysis_orders.mode` 為自由文字（無 enum），標準值 = `['HD','SLED','CVVHDF','PP','DFPP','Lipid']`（對齊前端 `patient-form-modal` MODES）。

- **共用 helper** (`utils/dialysisMode.js`): `normalizeDialysisMode`（`SLEDD`/`SLEDF`→`SLED`、大小寫不敏感、去空白、未知值保留）/ `normalizeDialysisOrdersMode`。
- **套用於所有寫 mode 的後端路徑**: `patients.js toDbFormat`、`orders.js POST /history`（DialysisOrderModal 儲存主路徑）、`scheduler.js UPDATE_*`。
- 注意：使用者口語的「SLEDD」在資料庫實際存為 **`SLED`**（單一 D）。
