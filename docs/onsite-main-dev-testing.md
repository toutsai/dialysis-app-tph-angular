...
6. **季度草稿**：同時修改兩位病人的季度資料，立刻切到下一季或跨年度，再切回檢查。內容應留在原季度；儲存失敗時能重試，不能寫到新季度。此項涵蓋護理季度輸入、HD 處方與住院季度表。
7. **多人日誌**：使用兩個不同帳號（各自登入不同瀏覽器），先後開啟同一天的日誌。另一人新增動態後，舊畫面存檔應提示衝突並保留草稿，不能刪掉新動態與已填 KiDit 表單。只存病人動態不能清除其他區塊的未存提示。
8. **護理職責**：修改一般規則、日班、128 班、夜班與合作分工，儲存後重新載入。內容應維持；修改後切換頁面／月份／分頁／按重新載入時，應先保護未存內容。
9. **Excel 處方**：使用模擬資料檢查模式改變時的當日快照、未來排程與歷史；新處方不被較舊匯入覆蓋，空白值與去重規則不變。
10. **登入失效**：登出、同帳號另一處登入、停用或修改角色後，舊即時連線應停止；使用兩個不同帳號測多人情境，避免既有單一裝置規則互相登出。
11. **查詢結果**：開啟不同病人的醫囑歷史，確認仍依更新時間排列、最多顯示 20 筆；切換檢驗警示月份，跨年、月底與二月的結果應符合選取範圍，舊請求不能覆蓋新結果。
12. **Excel 與表格**：上傳合成檔案時可另開頁面查看資料；系統忙碌或解析逾時會提示重試。護理月表／週表切換月份、修改班別後，日期、護理師與分組應即時正確顯示。背景解析改善主程式回應，不保證整份匯入時間縮短。

庫存管理依使用者要求暫不修改。預約生效日 01:00、逾期不補跑、今日凍結、KiDit 排除「更改模式／勿動」與既有頻率、模式規則均保留。必要更新的交易保護已涵蓋六種預約類型；故障回滾與重試由合成資料自動測試驗證。HIS 傳輸介面、Angular 大版本升級及完整院內效能量測仍另行安排。

GitHub workflow 對 Linux／Windows 執行安裝、語法、回歸、建置與 API smoke。測試通過後，仍需你確認主機畫面和作業結果，再決定是否合併回 `onsite-main`；本分支本身不會更新正式站。

本輪修正、效能量測與驗證範圍見 [第一、二批結果](onsite-main-dev-stage1-stage2-results.md)。
