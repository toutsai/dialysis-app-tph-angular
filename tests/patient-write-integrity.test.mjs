...
try {
  ...
  // 醫囑 Excel 上傳刻意不做病人連動（不快照/不寫動態/不取消調班；連動只掛病人清單 PUT，2026-09-03 裁定）→ 失敗注入改放在歷史表
  before=tableState(); eventCount=events.length
  db.exec("CREATE TRIGGER fail_history BEFORE INSERT ON dialysis_orders_history BEGIN SELECT RAISE(ABORT,'synthetic failure'); END")
  ...
  assert.equal(db.prepare("SELECT count(*) AS n FROM patient_history WHERE patient_id='upload' AND event_type='MODE_CHANGE'").get().n,0)
  ...
  cases.push('XLSX archive/current/history rollback; no patient linkage (snapshot/exception/history untouched); retry, dedup, newer-wins, KiDit exclusion')
  patient('normalize')
