...
// 護理分組歷史快照：列出某日快照
export async function fetchNurseAssignmentRevisions(dateStr: string): Promise<any[]> {
  try {
    const data = await localApi.get(`${ROUTE}/${dateStr}/revisions`);
    return Array.isArray(data) ? data : (data?.data || []);
  } catch (error) {
    console.error('Failed to fetch nurse assignment revisions:', error);
    throw error;
  }
}

// 護理分組歷史快照：還原指定快照（後端會先快照現況）
export async function restoreNurseAssignmentRevision(dateStr: string, revisionId: string): Promise<any> {
  try {
    return await localApi.post(`${ROUTE}/${dateStr}/revisions/${revisionId}/restore`, {});
  } catch (error) {
    console.error('Failed to restore nurse assignment revision:', error);
    throw error;
  }
}
