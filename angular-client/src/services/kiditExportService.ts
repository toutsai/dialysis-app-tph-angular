// src/services/kiditExportService.ts
// Backward-compatible bridge file (Phase 5 migration)
// Re-exports exportKiDitExcel that components still import from '@/services/kiditExportService'
import * as XLSX from 'xlsx';
import { toRocDate } from '@/utils/kiditHelpers';

export function exportKiDitExcel(logbookEvents: any[], filename: string = 'KiDit_Export.xlsx'): void {
  const sheetData: { patients: any[]; history: any[]; access: any[] } = {
    patients: [],
    history: [],
    access: [],
  };
  ...
  logbookEvents.forEach((event) => {
    ...
    if (!processedPatientIds.has(pid)) {
      ...
      const mapCheckboxes = (selectedIndices: number[], totalBits: number): string => {
      ...
      sheetData.history.push({
        ...
        '19 腎臟超音波檢查異常': h.renalUltrasoundAbnormal || '',
        '20 腎臟超音波檢查異常說明': h.renalUltrasoundDesc || '',
        '腎臟超音波檢查異常其他說明': h.renalUltrasoundOtherDesc || '',
        '21 腎臟超音波檢驗日期': toRocDate(h.renalUltrasoundDate),
        '22 其他系統性疾病': mapCheckboxes(h.selectedSystemicDiseases, 15),
        ...
        '28 BUN(mg/dl)': h.initialLabBUN || h.initialBUN || '',
        '29 Creatinine(mg/dl)': h.initialLabCr || h.initialCr || '',
        '30 K(meq/l)': h.initialK || '',
        '31 CCr(ml/min)': h.initialCCr || '',
        ...
        '36 其他症狀': mapCheckboxes(h.selectedSymptoms, 15),
        '37 其他症狀(其他)': h.symptomsOtherDescription || '',
        '38 緊急透析原因': mapCheckboxes(h.selectedEmergencyReasons, 20),
        '39 緊急透析原因(其他)': h.emergencyReasonsOtherDescription || '',
        '40 檢驗日期': toRocDate(h.emergencyLabDate),
        '41 Hct (%)': h.emergencyHct || '',
        '42 Hb (g/dl)': h.emergencyHb || '',
        '43 BUN (mg/dl)': h.emergencyBUN || '',
        '44 Creatinine (mg/dl)': h.emergencyCr || '',
        '45 CCr (ml/min)': h.emergencyCCr || '',
        '46 Na (mg/dl)': h.emergencyNa || '',
        '47 K (meq/l)': h.emergencyK || '',
        '48 HCO3 (meq/l)': h.emergencyHCO3 || '',
        '49 Albumin(gm/dl)': h.emergencyAlb || '',
