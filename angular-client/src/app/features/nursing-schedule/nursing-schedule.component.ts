...
export class NursingScheduleComponent implements OnInit {
  ...
  getAvailableGroups(shift: string, date: string, nurseId: string): string[] {
    ...
    // 128 班：白班組往後加一組（K 空則 K、K 滿則 L），不占夜班組池
    if (this.is128Shift(s)) {
      return ['K', 'L', 'M'];
    }
    ...
    Object.entries(this.tempScheduleWithGroups.scheduleByNurse).forEach(
      ([otherId, otherData]: [string, any]) => {
        ...
        if (otherGroup === newGroup) {
          ...
          // 128 = 白班組往後加一組：與白班同組或 128 彼此同組都算衝突；與夜班同組不衝突
          ...
          if (
            ...
            (isCurrent128 && (isOther128 || isOtherDayShift)) ||
            (isCurrentDayShift && isOther128)
          ) {
  ...
  async processAndUpload(): Promise<void> {
    ...
    try {
      ...
      const res = await fetch(`${this.firebase.apiBaseUrl}/nursing/save-schedule`, {
        method: 'POST',
        headers: this.firebase.getHeaders(),
        body: JSON.stringify(payload),
      });
      const resultData: any = await res.json();
      if (!resultData.success)
        throw new Error(resultData.message || '處理失敗');
      this.uploadStatus.set(`成功！${resultData.message}`);
