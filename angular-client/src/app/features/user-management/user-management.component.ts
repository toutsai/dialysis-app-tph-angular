  ...
  async handleSaveUser(userData: any): Promise<void> {
    ...
    try {
      if (this.isEditing()) {
        ...
        // If password is being changed, include it in the update request
        const dataToSend = password ? { ...updateData, password } : updateData;

        try {
          const res = await fetch(`${this.firebase.apiBaseUrl}/auth/users/${id}`, {
            method: 'PUT',
          ...
          if (password && error.message?.includes('密碼')) {
            this.showAlert('密碼更新失敗', error.message || '密碼需至少 8 個字元，並包含大寫字母、小寫字母和數字。');
        ...
        try {
          const res = await fetch(`${this.firebase.apiBaseUrl}/auth/users`, {
            method: 'POST',
            headers: this.firebase.getHeaders(),
          ...
          // Refresh user list to get the new user
