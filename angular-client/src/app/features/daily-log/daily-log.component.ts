...
export class DailyLogComponent implements OnInit, OnDestroy {
  ...
  async loadDailyLog(dateStr: string): Promise<void> {
    ...
    try {
      ...
      if (logResult) {
        const initial = this.initialLogState();
        const logData = logResult as any;

        // Deep merge stats: server data might have partial stats
        const mergedStats = {
          main_beds: { ...initial.stats.main_beds, ...(logData.stats?.main_beds || {}) },
          peripheral_beds: { ...initial.stats.peripheral_beds, ...(logData.stats?.peripheral_beds || {}) },
          patient_care: {
            onDL: { ...initial.stats.patient_care.onDL, ...(logData.stats?.patient_care?.onDL || {}) },
            akChange: { ...initial.stats.patient_care.akChange, ...(logData.stats?.patient_care?.akChange || {}) },
            noShow: { ...initial.stats.patient_care.noShow, ...(logData.stats?.patient_care?.noShow || {}) },
          },
          staffing: { ...initial.stats.staffing, ...(logData.stats?.staffing || {}) },
        };
        // Deep merge each shift in main_beds and peripheral_beds
        for (const shift of ['early', 'noon', 'late']) {
          mergedStats.main_beds[shift] = { ...initial.stats.main_beds[shift], ...(logData.stats?.main_beds?.[shift] || {}) };
          mergedStats.peripheral_beds[shift] = { ...initial.stats.peripheral_beds[shift], ...(logData.stats?.peripheral_beds?.[shift] || {}) };
        }

        // Deep merge leader
        const mergedLeader = {
          early: { ...initial.leader.early, ...(logData.leader?.early || {}) },
          noon: { ...initial.leader.noon, ...(logData.leader?.noon || {}) },
          late: { ...initial.leader.late, ...(logData.leader?.late || {}) },
        };

        const mergedLog = { ...initial, ...logData, stats: mergedStats, leader: mergedLeader };
  ...
  async checkAndSyncBeforeSave(onComplete: () => Promise<void>): Promise<void> {
    try {
      const allSchedules = await this.schedulesApi.fetchAll();
      const scheduleData = allSchedules.filter((s: any) => s.date === this.selectedDate());
      if (scheduleData.length === 0) {
        await onComplete();
        return;
      }
