  ...
  async generateReport(): Promise<void> {
    ...
    try {
      ...
      if (this.reportType() === 'staffing_monthly') {
        ...
        if (endDate < todayStr) {
          fetchPromises.push(
            this.expiredSchedulesApi.fetchAll().then(data => {
              expiredSchedulesData = data.filter((d: any) => d.date >= startDate && d.date <= endDate);
            })
          );
        } else if (startDate >= todayStr) {
          fetchPromises.push(
            this.schedulesApi.fetchAll().then(data => {
              schedulesData = data.filter((d: any) => d.date >= startDate && d.date <= endDate);
            })
          );
        } else {
          fetchPromises.push(
            this.expiredSchedulesApi.fetchAll().then(data => {
              expiredSchedulesData = data.filter((d: any) => d.date >= startDate && d.date < todayStr);
            })
          );
          fetchPromises.push(
            this.schedulesApi.fetchAll().then(data => {
              schedulesData = data.filter((d: any) => d.date >= todayStr && d.date <= endDate);
            })
          );
        }
