...
export class CollaborationComponent implements OnInit, OnDestroy {
  ...
  private bulletinPollingTimer: any = null;
  ...
  openEducationRecord(msg: TaskItem): void {
    if (!msg?.patientId) return;
    this.educationPatient.set({ id: msg.patientId, name: msg.patientName || '' });
  ...
  ngOnDestroy(): void {
    if (this.bulletinPollingTimer) {
      clearInterval(this.bulletinPollingTimer);
      this.bulletinPollingTimer = null;
    }
  }
  ...
  formatTimestamp(ts: unknown): string {
    if (!ts) return '';
    const date = parseFirestoreTimestamp(ts as string | number | Date);
  ...
  async handleSaveAnnouncement(): Promise<void> {
    ...
    try {
      // Fetch existing log, append announcement, save back
  ...
  private async listenToBulletinData(dateStr: string): Promise<void> {
    ...
    await Promise.all([fetchLastWorkingDayLog(), fetchBulletin()]).finally(() => {
  ...
  private async refreshBulletin(dateStr: string): Promise<void> {
    try {
      const todayLog: any = await this.logsApi.fetchById(dateStr);
      if (todayLog && todayLog.announcements) {
        const sorted = [...todayLog.announcements].sort(
          (a: any, b: any) => {
        ...
        this.todaysAnnouncements.set(sorted);
      } else {
        this.todaysAnnouncements.set([]);
      }
    } catch (error) {
  ...
  private async loadDailyPatientData(date: string): Promise<void> {
    ...
    try {
      ...
      // 直接以日期 fetchById 取當日排程與護理分配 (對齊可運作的 my-patients 與舊 Vue 版)。
      // 先前用 schedulesApi.fetchAll().filter(date) 會因 schedules list 端點回傳精簡/空資料
      // 而 schedules.length===0 提早 return，導致整個左欄(含「所有病人」)都是空的。
      const [scheduleRecord, assignmentRecord] = await Promise.all([
        this.schedulesApi.fetchById(date),
        this.assignmentsApi.fetchById(date).catch(() => null),
      ]);
      const scheduleData = ((scheduleRecord as Record<string, unknown>)?.['schedule'] ||
      ...
      if (assignmentRecord) {
        const assignmentData = assignmentRecord as Record<string, unknown>;
        // nurse_assignments 的 names 與真正的 teams map 都包在 teams JSON 欄位內
        // (schema 只有 teams 欄位)，需解包；同時相容可能的扁平結構。
        const rawTeams = assignmentData['teams'] as Record<string, any> | undefined;
        const payload =
          rawTeams && (rawTeams['names'] || rawTeams['teams']) ? rawTeams : assignmentData;
        const names = payload['names'] as Record<string, string> | undefined;
        const teams = payload['teams'] as Record<string, Record<string, string>> | undefined;
        if (names && teams) {
          const userName = (((user as Record<string, unknown>)['name'] as string) || '').trim();
          ...
          for (const key in teams) {
            const ta = teams[key] || {};
            if (
              myTeamNames.has(ta['nurseTeam']) ||
              myTeamNames.has(ta['nurseTeamIn']) ||
              myTeamNames.has(ta['nurseTeamOut'])
            ) {
