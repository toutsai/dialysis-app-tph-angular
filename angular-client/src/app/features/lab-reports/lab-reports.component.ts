...
export class LabReportsComponent implements OnInit, OnDestroy {
  ...
  async generateAlertReport(): Promise<void> {
    ...
    try {
      ...
      for (const patient of allOpdPatients) {
        const allPatientReports = await this.labReportsApi.fetchAll();
        const reports = allPatientReports
          .filter((r: any) => 
            r.patientId === patient.id &&
            r.reportDate >= startDate.toISOString().slice(0, 10) &&
            r.reportDate < endDate.toISOString().slice(0, 10)
          )
  ...
  async findMissingPatients(): Promise<void> {
    ...
    try {
      ...
      // Use patientStore instead of Firestore documentId query
      const pMap = this.patientStore.patientMap();
      const missingPatientDetails: any[] = missingIds
        .map((id) => pMap.get(id))
