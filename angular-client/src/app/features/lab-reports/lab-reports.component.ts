...
export class LabReportsComponent implements OnInit, OnDestroy {
  ...
  // 檢驗套組規則：依「查詢月份」決定該月應驗項目（key = 報告 data 短碼）
  // label = 補登 Excel 的「細項名稱」，後端 LAB_ITEM_MAPPING 可解析回該短碼
  readonly LAB_ITEM_LABELS: Record<string, string> = {
    BUN: 'BUN(Blood)',
    Hb: '血色素', // CBC 以血色素(Hb)為代表
    UricAcid: '尿酸(血)',
    Na: '血中鈉',
    K: '血中鉀',
    Ca: 'Calcium(Blood)',
  ...
  // AST/GOT、Alkaline phosphatase 不納入比對（系統未存）
  private readonly MONTHLY_ITEMS = ['BUN', 'Hb', 'UricAcid', 'Na', 'K', 'Ca', 'P', 'Albumin', 'ALT', 'GlucoseAC', 'PostBUN', 'Creatinine'];
  ...
  private requiredItemsForMonth(monthNum: number): string[] {
    if (monthNum === 3 || monthNum === 9) return this.QUARTERLY_ITEMS;
    if (monthNum === 6) return this.SEMI_ANNUAL_ITEMS;
    if (monthNum === 12) return this.ANNUAL_ITEMS;
    return this.MONTHLY_ITEMS; // 1,2,4,5,7,8,10,11
  }
    ...
    try {
      ...
      for (const r of allReports) {
        if (r.reportDate >= startStr && r.reportDate < endStr) {
          const arr = reportsByPatient.get(r.patientId);
          if (arr) arr.push(r);
          else reportsByPatient.set(r.patientId, [r]);
        }
      }

      // requiredMonths 與病人無關，迴圈外算一次
      const requiredMonths = [
        range.end,
        new Date(
          new Date(range.end + '-01').setMonth(new Date(range.end + '-01').getMonth() - 1),
        )
          .toISOString()
          .slice(0, 7),
        range.start,
      ...
      for (const patient of allOpdPatients) {
        const reports = (reportsByPatient.get(patient.id) || [])
        ...
        if (abnormalities.length > 0) {
          ...
          // 頻率與班別/床號一致，優先取基準排程規則；缺則退回病人本身 freq
          patientDataForReport.freq = scheduleInfo?.freq || (patient as any).freq || 'N/A';
      ...
      if (patientIdsInList.length > 0) {
        ...
        // Local filter by monthRange
        const filteredAnalyses = savedAnalyses.filter((a: any) => a.monthRange === monthRangeKey);
  ...
  private async searchIndividualReports(): Promise<void> {
    ...
    const reportsRaw = allReports
      .filter((r: any) => {
        ...
        const rd = typeof r.reportDate === 'string' ? r.reportDate : (r.reportDate?.toDate ? r.reportDate.toDate().toISOString().slice(0, 10) : '');
        return rd >= startDateStr && rd < endDateStr;
      })
      .sort((a: any, b: any) => {
        const rdA = typeof a.reportDate === 'string' ? a.reportDate : (a.reportDate?.toDate ? a.reportDate.toDate().toISOString().slice(0, 10) : '');
        const rdB = typeof b.reportDate === 'string' ? b.reportDate : (b.reportDate?.toDate ? b.reportDate.toDate().toISOString().slice(0, 10) : '');
  ...
  async handleUpload(): Promise<void> {
    ...
    try {
      ...
      const res = await fetch(`${this.firebaseService.apiBaseUrl}/lab-reports/process`, {
        method: 'POST',
  ...
  async findMissingPatients(): Promise<void> {
    ...
    try {
      ...
      // 逐項比對：依查詢月份取應驗套組，聯集該病人該月所有報告「已有值」的項目
      const requiredKeys = this.requiredItemsForMonth(month);
      const haveByPatient = new Map<string, Set<string>>();
      for (const r of reportsInMonth as any[]) {
        let have = haveByPatient.get(r.patientId);
        if (!have) {
          have = new Set<string>();
          haveByPatient.set(r.patientId, have);
        }
        const d = r.data || {};
        for (const k in d) {
          const v = d[k];
          if (v !== null && v !== undefined && String(v).trim() !== '') have.add(k);
        }
      }

      const pMap = this.patientStore.patientMap();
      const result: any[] = [];
      for (const id of allPatientIdsInGroup) {
        const have = haveByPatient.get(id);
        const missingKeys = requiredKeys.filter((k) => !have || !have.has(k));
        if (missingKeys.length === 0) continue;
        const patientData = pMap.get(id);
        if (!patientData) continue;
        const labData: Record<string, string> = {};
        const missingItems = missingKeys.map((k) => {
          labData[k] = '';
          return { key: k, label: this.LAB_ITEM_LABELS[k] || k };
        });
        result.push({
          id: patientData.id,
      ...
      this.missingPatients.set(result);
    } catch (error) {
  ...
  async generateAndUploadManualData(): Promise<void> {
    ...
    this.missingPatients().forEach((patient) => {
      (patient.missingItems || []).forEach((item: { key: string; label: string }) => {
