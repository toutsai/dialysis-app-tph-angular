...
export class PatientStoreService {
  ...
  private async loadPatients(): Promise<void> {
    ...
    try {
      ...
      // freq 真理之源是 MASTER_SCHEDULE 的 scheduleRule.freq；
      // 舊頂層 freq（來自 dialysis_orders.freq）覆蓋率偏低，僅作 fallback
      const patientsWithRules: Patient[] = patients.map((patient) => {
        const scheduleRule = (rulesMap.get(patient.id!) as ScheduleRule) ?? null;
        return {
          ...patient,
          scheduleRule,
          freq: scheduleRule?.freq || (patient as any).freq || '',
        };
      });
