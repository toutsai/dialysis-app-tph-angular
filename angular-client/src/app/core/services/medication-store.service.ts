...
export class MedicationStoreService {
  ...
  async fetchDailyInjections(
    ...
    try {
      ...
      const enrichedRecords = Array.from(latestMap.values())
        .filter(order => order.dose && order.dose !== '0' && order.dose !== '')
        .filter(order => this.shouldAdministerOnDate(order.note || '', date))
        .map(order => {
  ...
  /**
   * Query injection orders from REST API.
   * Replaces the Firestore compound query with a single REST call.
   */
  private async queryInjectionOrders(
    ...
    try {
      const res = await fetch(`${this.firebaseService.apiBaseUrl}/medications/injections`, {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        // Add patientIds as comma-separated
        params.set('patientIds', patientIds.join(','));

        const fallbackRes = await fetch(
          `${this.firebaseService.apiBaseUrl}/medications/injections?${params}`,
          { headers: this.firebaseService.getHeaders() },
        );
      ...
      console.error('[MedicationStoreService] queryInjectionOrders error:', error);
      return [];
    }
  }
