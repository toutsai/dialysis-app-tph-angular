...
export class LabMedCorrelationViewComponent implements OnChanges {
  ...
  async fetchData() {
    ...
    try {
      ...
      const patientReports = (allReports as any[]).filter((r: any) => r.patientId === this.patient.id);
      const patientOrders = (allOrders as any[]).filter((o: any) => o.patientId === this.patient.id);
      const patientDrafts = (allDrafts as any[]).filter((d: any) => d.patientId === this.patient.id);
