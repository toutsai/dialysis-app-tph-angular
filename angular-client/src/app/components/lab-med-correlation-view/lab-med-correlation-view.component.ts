...
export class LabMedCorrelationViewComponent implements OnChanges {
  ...
  // 衍生資料：在 fetchData() 完成後計算一次並快取，避免每次變更偵測重算
  // (原本是 getter，會在每個 CD cycle 被巢狀表格重複呼叫上百次造成遲滯)
  labReportMonths: string[] = [];
  latestLabMonth = '';
  draftTargetMonth = '';
  processedLabs: Record<string, Record<string, number>> = {};
  processedOrders: Record<string, Record<string, any>> = {};
  ...
  /**
   * 重新計算所有衍生資料並快取到欄位。
   * 只在 fetchData() / patient 變更時呼叫一次，而非每個變更偵測週期。
   */
  private recomputeDerived(): void {
    const ctm = this.currentTargetMonth;

    // labReportMonths
    if (!this.rawLabReports.length && !this.rawMedOrders.length && !this.rawMedDrafts.length) {
      this.labReportMonths = [];
    } else {
      ...
      const addMonthIfValid = (month: string) => {
        if (!month) return;
        if (month <= ctm) monthSet.add(month);
      };
      ...
      this.labReportMonths = Array.from(monthSet).sort().reverse().slice(0, 12);
    }
    this.latestLabMonth = this.labReportMonths[0] || '';
    this.draftTargetMonth = this.labReportMonths[0] || ctm;

    // processedLabs
    const labs: Record<string, Record<string, number>> = {};
    this.rawLabReports.forEach(report => {
      const monthKey = report.reportDate.slice(0, 7);
      for (const itemKey in report.data) {
        if (!labs[itemKey]) labs[itemKey] = {};
        if (!labs[itemKey][monthKey]) labs[itemKey][monthKey] = report.data[itemKey];
      }
    });
    this.processedLabs = labs;

    // processedOrders
    const data: Record<string, Record<string, any>> = {};
    const medByCode = new Map<string, MedDef>(this.allMedsMaster.map(m => [m.code, m]));
    ...
    sortedOrders.forEach(order => {
      ...
      const medInfo = medByCode.get(order.orderCode);
  ...
  async fetchData() {
    ...
    try {
      ...
      // 伺服器端已用 patientId 過濾，前端不需再 filter 一次
      const pid = this.patient.id;
      const [allReports, allOrders, allDrafts] = await Promise.all([
        labReportsApi.fetchWhere({ patientId: pid }),
        ordersApi.fetchWhere({ patientId: pid }),
        draftOrdersApi.fetchWhere({ patientId: pid }),
      ]);

      const patientReports = allReports as any[];
      const patientOrders = allOrders as any[];
      const patientDrafts = allDrafts as any[];
      ...
      this.rawMedDrafts = patientDrafts.filter((d: any) => d.status === 'pending');

      // 一次性計算衍生資料並快取（取代 template getter，消除遲滯）
      this.recomputeDerived();
    } catch (err: any) {
  ...
  async saveDraftOrders() {
    ...
    try {
      ...
      // Delete old drafts for this patient + month
      const allDrafts = await draftOrdersApi.fetchAll();
      const oldDrafts = (allDrafts as any[]).filter((d: any) =>
        d.patientId === this.patient.id && d.targetMonth === this.draftTargetMonth
      );
      await Promise.all(oldDrafts.map((d: any) => draftOrdersApi.delete(d.id)));
      ...
      for (const medCode in this.orderDraft) {
        ...
        if (draft.dose || draft.frequency) {
          ...
          createPromises.push(draftOrdersApi.create({
