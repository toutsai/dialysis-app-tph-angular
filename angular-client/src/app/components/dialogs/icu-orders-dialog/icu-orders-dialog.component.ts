...
export class IcuOrdersDialogComponent implements OnChanges {
  ...
  /** 依選取日期生效的醫囑覆蓋表 patientId -> orders（無者沿用病人現行 dialysisOrders） */
  @Input() effectiveOrders: Record<string, any> = {};
    ...
    return Object.entries(this.schedule)
      ...
      .map(([shiftId, slot]: [string, any]) => {
        ...
        if (
          ...
          return {
            ...patient,
            dialysisOrders: this.effectiveOrders[patient.id] || patient.dialysisOrders,
