...
export class ExceptionCreateDialogComponent implements OnChanges {
  ...
  private readonly exceptionsApi: ApiManager<FirestoreRecord>;
  ...
  /** 預填新增（非編輯）：拖曳換班/換床轉調班時帶入 type/patient/from/to，保留目標床位 */
  ...
  constructor() {
    this.exceptionsApi = this.apiManagerService.create<FirestoreRecord>('schedule_exceptions');
  ...
  async handleDelete(): Promise<void> {
    ...
    try {
      await this.exceptionsApi.delete(this.initialData.id);
  ...
  async fetchSourceSchedule(): Promise<void> {
    ...
    try {
      const scheduleDoc: any = await this.schedulesApi.fetchById(date);
      if (!scheduleDoc) {
  ...
  async openBedAssignmentForTarget(): Promise<void> {
    ...
    try {
      const scheduleDoc: any = await this.schedulesApi.fetchById(targetDate);
      if (scheduleDoc) {
  ...
  async fetchScheduleForSwap(): Promise<void> {
    ...
    try {
      const scheduleDoc: any = await this.schedulesApi.fetchById(date);
      const schedule = scheduleDoc ? (scheduleDoc.schedule || null) : null;
